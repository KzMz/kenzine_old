...
ASSEMBLY := engine
EXTENSION := .wasm
COMPILER_FLAGS := -Werror -Wvla -Wgnu-folding-constant -Wno-missing-braces -fdeclspec -fno-builtin --target=wasm32 --no-standard-libraries -Wl,--no-entry -Wl,--export=__heap_base -Wl,--allow-undefined #-fPIC
INCLUDE_FLAGS := -Iengine\src
LINKER_FLAGS := -L$(OBJ_DIR)\engine
...
$(OBJ_DIR)/%.c.o: %.c # compile .c to .c.o object
	@echo   $<...
	@clang $< --target=wasm32 $(COMPILER_FLAGS) -o $@ $(DEFINES) $(INCLUDE_FLAGS)
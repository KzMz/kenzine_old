...
bool test_arena_alloc_clear(void)
{
    ...
    expect_eq(arena_get_max_size(&arena), arena_get_region_size());
...
{
    Arena arena = {0};
    u8* alloc = arena_alloc(&arena, arena_get_region_size() * 2, false);
    expect_not_eq(alloc, NULL);
    expect_eq(arena.num_allocations, 1);
    expect_eq(arena.num_dynamic_allocations, 1);
    expect_eq(arena_get_size(&arena), arena_get_region_size() * 2);
    expect_eq(arena_get_max_size(&arena), arena_get_region_size() * 2);
    ...
    expect_eq(arena_get_size(&arena), arena_get_region_size() * 2 + sizeof(u8) * 10);

    u8* alloc3 = arena_alloc(&arena, sizeof(u8) * 2, false);
    expect_not_eq(alloc3, NULL);
    expect_eq(arena.num_allocations, 3);
    expect_eq(arena.num_dynamic_allocations, 2);
    expect_eq(arena_get_size(&arena), arena_get_region_size() * 2 + sizeof(u8) * 10 + sizeof(u8) * 2);
